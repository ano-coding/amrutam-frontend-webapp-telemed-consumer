      ...
      <section className="mt-12 flex max-w-full items-center justify-center">
        <div className="flex w-3/4 flex-wrap justify-end">
          {paramsArray.map((choice, i) => (
            <div
              key={i}
              className="bg-primary-tint-2 font-inter mx-2 my-2 flex h-[2.813rem] shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-[31px] px-5 py-[0.937rem] text-center text-[1rem] tracking-[0.5px] text-black"
            >
              <span className="capitalize">
                {choice[0] === "specialities"
                  ? filteredSpecialities?.find((s) => s.value === choice[1])
                      .label
                  : choice[1]}
              </span>
              <button
              ...
              >
                <svg
                  ...
                  strokeWidth={1.5}
                  stroke="currentColor"
                  className="h-6 w-6 cursor-pointer text-[#7b7b7b] hover:scale-110 hover:text-gray-600"
                >
                  <path
                    strokeLinecap="round"

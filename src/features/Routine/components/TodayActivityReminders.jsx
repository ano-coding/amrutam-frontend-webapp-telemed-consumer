...
const TodayActivityReminders = ({ reminder }) => {
  return (
    <>
      <div className="flex gap-3">
        <img
          ...
          alt=""
          className="size-[50px] rounded-lg border object-cover"
        />
        ...
        <div className="flex flex-col gap-[9px]">
          ...
          <div className="flex flex-col text-[12px] font-medium capitalize tracking-[-0.02em] text-[#a0a0a0] sm:flex-row sm:items-center sm:gap-4">
            <div>{reminder?.activityType}</div>
            {reminder?.timeslotActivityBased?.map((timeSlot) => (
...
export default TodayActivityReminders;

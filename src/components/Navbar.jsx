...
const Navbar = () => {
  ...
  const { setToggled } = useContext(SideBarToggleContext);
  return (
    <Disclosure as="nav">
      {({ open }) => (
        <>
                  ...
                  <div className="hidden w-full lg:flex">
                    <div className="flex w-full flex-row items-center justify-around">
                      <nav className="flex justify-between gap-5 lg:gap-10 xl:gap-16">
                        <Link to={"/"}>
                          <img className="w-56" src="/amrutam-text-logo.png" />
                        </Link>
                      ...
                      <div className="flex gap-4 lg:ml-2">
                        <Link
                          to="/login"
                          className="rounded-xl border-2 border-[#3a643b] px-8 pb-3 pt-2 text-2xl text-[#3a643b] transition duration-100 ease-in-out hover:-translate-y-0.5 hover:border-[#4e8750] hover:text-[#4e8750] active:scale-90"
                        >
                          Login
                        </Link>
                        <Link
                          to="/signup"
                          className="whitespace-nowrap rounded-xl bg-[#3a643b] px-8 pb-3 pt-2 text-2xl text-white transition delay-75 duration-100 ease-in-out hover:-translate-y-0.5 hover:bg-[#4e8750] active:scale-90"
                        >
                          Sign-Up
                        </Link>
                      </div>
                  ...
                  <div ref={ref} className="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                    {NAV_LINKS.map((item) => (
                      <NavLink to={item.path} key={item.path}>
                        {({ isActive }) => (
                          <DisclosureButton
                            className={`w-full text-left ${isActive ? `font-semibold text-[#3a643b]` : "text-neutral-700"} block rounded-md px-7 py-2 text-lg capitalize hover:text-black`}
                          >
                            {item.name}
                          </DisclosureButton>
                        )}
